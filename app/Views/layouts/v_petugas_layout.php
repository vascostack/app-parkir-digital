    ...
    <div class="position-absolute bottom-0 w-100 p-4 border-top bg-white">
      <div class="d-flex align-items-center gap-3">
        ...
        <div class="lh-1 overflow-hidden">
          <span class="d-block font-poppins fw-bold text-navy-dark text-truncate text-capitalize">
            <?= esc(session()->get('nama_user') ?? session()->get('username') ?? 'Petugas') ?>
          </span>
          <small class="text-muted d-block" style="font-size: 11px;">
            <?= esc(session()->get('role') ?? 'Shift Aktif') ?>
          </small>
        </div>
      </div>
    </div>
